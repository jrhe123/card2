...
public class PackOpeningArea : MonoBehaviour
{
    ...
    public GameObject ExperienceCardFromPackPrefab;
    public GameObject WeaponCardFromPackPrefab;
    ...
    private GameObject CardFromPack(RarityOptions rarity)
    {
        ...
        a.UpdateCardAssetByAccumulativeExperience(500);
        CardCollection.Instance.OwnedCardList.Add(a);

        GameObject card;
        if (a.TypeOfCard == TypesOfCards.Creature)
            card = Instantiate(CreatureCardFromPackPrefab) as GameObject;
        else if (a.TypeOfCard == TypesOfCards.Experience)
            card = Instantiate(ExperienceCardFromPackPrefab) as GameObject;
        else if (a.TypeOfCard == TypesOfCards.Weapon)
            card = Instantiate(WeaponCardFromPackPrefab) as GameObject;
        else
            card = Instantiate(CreatureCardFromPackPrefab) as GameObject;
